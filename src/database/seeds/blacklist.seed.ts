...
export default class BlacklistSeed implements Seeder {
  public async run(): Promise<any> {
    let id = 0;
    // create all the email type blacklists
    for (const email of this.BlackListItems.emails) {
      const blacklist = new Blacklist();
      blacklist.id = ++id;
      blacklist.blacklistName = email;
      blacklist.type = 'email';
      await blacklist.save();
    }
    // create all the username type blacklists
    for (const username of this.BlackListItems.usernames) {
      const blacklist = new Blacklist();
      blacklist.id = ++id;
      blacklist.blacklistName = username;
      blacklist.type = 'username';
      await blacklist.save();
    }
  }
